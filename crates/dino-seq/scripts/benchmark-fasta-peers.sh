...
project_dir="${DINO_SEQ_FASTA_PEER_PROJECT_DIR:-target/fasta-peer-bench}"
out_dir="${DINO_SEQ_FASTA_PEER_OUT_DIR:-docs/benchmarks/fasta-peers}"
iters="${DINO_SEQ_FASTA_PEER_ITERS:-5}"
records="${DINO_SEQ_FASTA_PEER_RECORDS:-100000}"
read_len="${DINO_SEQ_FASTA_PEER_READ_LEN:-150}"
input="${DINO_SEQ_FASTA_PEER_INPUT:-}"
...
dino_seq_features="${DINO_SEQ_FASTA_PEER_DINO_SEQ_FEATURES:-}"
cargo_command="${DINO_SEQ_FASTA_PEER_CARGO:-cargo}"
consumer="${DINO_SEQ_FASTA_PEER_CONSUMER:-light}"
compression="${DINO_SEQ_FASTA_PEER_COMPRESSION:-raw}"
...
DINO_SEQ_FASTA_PEER_CONSUMER="${consumer}" "${cargo_cmd[@]}" run --release --manifest-path "${project_dir}/Cargo.toml" -- "${args[@]}"
...

{
  ...
  printf -- '- dino_seq_batch_records: %s\n' "${DINO_SEQ_FASTA_PEER_BATCH_RECORDS:-4096}"
  if [[ -n "${input}" ]]; then
    printf -- '- input: %s\n' "$(dino_seq_display_path "${input}")"
  else
  ...
  awk '
    $1 == "name" {
      name = $3
      gsub(/"/, "", name)
      keep = name == "dino_seq" || name == "seq_io" || name == "bio" || name == "libdeflater"
    }
    keep && $1 == "version" {
      version = $3
      gsub(/"/, "", version)

...
out_dir="${DINO_SEQ_SIZE_SWEEP_OUT_DIR:-target/bench-results/rust-peer-size-sweep}"
sizes="${DINO_SEQ_SIZE_SWEEP_RECORDS:-10000 50000 100000 500000 1000000}"
read_len="${DINO_SEQ_SIZE_SWEEP_READ_LEN:-150}"
iters="${DINO_SEQ_SIZE_SWEEP_ITERS:-3}"
consumer="${DINO_SEQ_SIZE_SWEEP_CONSUMER:-light}"
compressions="${DINO_SEQ_SIZE_SWEEP_COMPRESSIONS:-raw}"
base_project_dir="${DINO_SEQ_SIZE_SWEEP_PROJECT_DIR:-target/rust-peer-size-sweep}"
...
for compression in ${compressions}; do
  for records in ${sizes}; do
    ...
    DINO_SEQ_RUST_PEER_OUT_DIR="${run_dir}" \
    DINO_SEQ_RUST_PEER_PROJECT_DIR="${project_dir}" \
    DINO_SEQ_RUST_PEER_RECORDS="${records}" \
    DINO_SEQ_RUST_PEER_READ_LEN="${read_len}" \
    DINO_SEQ_RUST_PEER_ITERS="${iters}" \
    DINO_SEQ_RUST_PEER_CONSUMER="${consumer}" \
    DINO_SEQ_RUST_PEER_COMPRESSION="${compression}" \
